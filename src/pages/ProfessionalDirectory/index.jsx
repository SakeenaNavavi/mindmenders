import React from 'react';
import './index.css';

const directory=()=>
{
    return (
        <div>
            <div className = "container">
                    <div className = "row justify-content-center">
                        <div className = "col-lg-6">
                            <h2 className = "text-center heading">GET IN TOUCH WITH A THERAPIST...</h2>
                            <p className="para">Discover Our Distinguished Network of Dedicated Mental Health Professionals</p>
                        </div>
                    </div>
            </div> 
            <div class="row row-cols-1 row-cols-md-5 g-4">
                <div class="column">
                    <div class="card">
                        <img src="https://nawinna.com/assets/images/doctor/100/187-untitled-1.jpg" alt="Jane" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://med.pdn.ac.lk/departments/psychiatry/staff/pabasari.png" alt="Mike" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Mike Ross</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://modernfamilymedicalcentre.com/wp-content/uploads/2021/11/Dr.-Lakshmi-Saranya-Marath-Thozhiyoor.jpg" alt="John" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. John Doe</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTXJmvF525gIJbdRibhaIkVa4DW-uov2z12Zg&usqp=CAU" alt="Jane" class="card-image"></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://almashospital.com/public/storage/doctor/of77Cr3c8q1asar5654DnHpSgew433FuEKVuzXFcX7.jpg" alt="Jane" class="card-image"></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://www.prshospital.com/img/doctors/advika_sivam_narayan.jpg" alt="Jane" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="data:image/jpeg;base64," alt="Jane" class="card-image"></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://kalyanasl.org/wp-content/uploads/2022/05/IMG_20220527_150639_240-Leila-A.-Tennakoon.jpg" alt="Jane" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://www.mayoclinic.org/-/media/kcms/employees/2018/10/28/15/50/eva-alden-20982127.jpg" alt="Jane" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
                <div class="column">
                    <div class="card">
                        <img src="https://photos.psychologytoday.com/353d8520-c394-4b99-ae48-b95ee3066d46/2/320x400.jpeg" alt="Jane" class="card-image" ></img>
                        <div class="container">
                            <h5 class="card-title">Dr. Rumi Ruben</h5>
                            <p class="title">Psychiatrist</p>
                            <p><button class="button-1">Book Now</button></p>
                            <p><button class="button">Chat Now</button></p>
                        </div>
                    </div>
                </div>
            </div>

        </div>                
           
    )
}
export default directory;